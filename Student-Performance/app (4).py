...
st.markdown(
    ...
    /* Keep every interactive control readable on the dark theme. */
    label, [data-testid="stWidgetLabel"] p {{ color: #ffffff !important; font-weight: 700 !important; }}
    [data-testid="stSlider"] p, [data-testid="stSlider"] span {{ color: {LIGHT_GOLD} !important; }}
    [data-testid="stSlider"] div[data-baseweb="slider"] div {{ color: {LIGHT_GOLD} !important; }}
    button[data-baseweb="tab"] p {{ color: #e8e8e8 !important; font-weight: 700 !important; }}
    button[data-baseweb="tab"][aria-selected="true"] p {{ color: {LIGHT_GOLD} !important; }}
    [data-testid="stCaptionContainer"] p {{ color: #bcbcbc !important; }}
    .hero {{
...
with predict_tab:
    ...
    with left:
        ...
        st.write("Choose the values first, then click the button to reveal the AI suggestion.")
        with st.form("student_profile_form"):
            ...
            submitted = st.form_submit_button("✨ Ask the AI", use_container_width=True)
    ...
    with right:
        ...
        selected = st.session_state.get("student_profile")
        if selected is None:
            st.markdown(
                "<div class='mini-card'><h3>Waiting for your choices…</h3>"
                "<p>Move the sliders and click <b>Ask the AI</b>. The prediction will appear here.</p></div>",
                unsafe_allow_html=True,
            )
        else:
            profile = pd.DataFrame([[
                selected["technical"], selected["aptitude"], selected["projects"],
                selected["interview"], selected["attendance"]
            ]], columns=features)
            ...
            support = []
            if selected["technical"] < 65: support.append("technical practice")
            if selected["aptitude"] < 65: support.append("aptitude practice")
            if selected["projects"] < 2: support.append("one portfolio project")
            if selected["interview"] < 65: support.append("mock interviews")
            if selected["attendance"] < 75: support.append("consistent participation")
            ...
            st.caption("Change the values and click Ask the AI again. When does the prediction change?")
